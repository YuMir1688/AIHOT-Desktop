...
Text(svc.Methods.Single(m=>m.Name=="Query"),"https://aihot.news/hot");
Text(module.Types.Single(t=>t.Name=="MainWindow").Methods.Single(m=>m.Name=="get_RangeLabel"),"48 小时热点 · 按热度排序");
...
foreach(var op in All(module.Types).SelectMany(t=>t.Methods).Where(m=>m.HasBody).SelectMany(m=>m.Body.Instructions).Where(i=>i.OpCode==OpCodes.Ldstr)){
 ...
 if(value.Contains("今天暂时没有精选资讯")||value.Contains("这个范围暂时没有精选资讯"))value="热榜暂时没有热点事件";
 value=value.Replace("精选 ","热点 ").Replace("AIHOT 精选","AIHOT 热点").Replace("有新精选后自动更新","榜单更新后自动刷新").Replace(" · 最多 100 条","");
 op.Operand=value;
}
module.Write(args[2]);Console.WriteLine("PASS: hot-feed methods patched; order preserved; today filter bypassed");
